...
from count_annotation_tool.sort_slice_name_lst import *
...
def save_cell1(event):
    ...
    pathToAnnotatedImage = os.path.join(save_folder,str(i % lenOfImages)+'.png')
    ...
    contours,hv2 = cv2.findContours(im,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    # for j in line_id_dict[i]:
    #     for k in j:
    #         canvas.delete(k)
    for l,y in enumerate(contours):
        if(cv2.pointPolygonTest(y,(markedx,markedy),True) > 0):
            print("found contour")
            dup_img = np.ones((width,height))
            cv2.drawContours(dup_img,[y],0,(0,0,0),-1)
            cv2.imshow("corrected particle",dup_img)
            and_img = np.logical_and(im,dup_img)
...
def draw_old_annotation(i):
    ...
    pathToAnnotatedImage = os.path.join(save_folder,str(i)+'.png')
    annotated_img=cv2.imread(pathToAnnotatedImage,-1)
    annotated_img = cv2.resize(annotated_img, (annotated_img.shape[1] * SCALE_FACTOR, annotated_img.shape[0] * SCALE_FACTOR), interpolation=cv2.INTER_NEAREST)
    ...
    cv2.imshow("image 2",disp_img)
    cv2.waitKey()
    cv2.drawContours(disp_img,contours,-1,(0,0,0),1)
    cv2.imshow("image 2",disp_img)
    cv2.waitKey()
    ...
    cv2.imwrite(os.path.join('./contoured_images',stack_name,str(i)+'.png'),disp_img)
...
def ReadSequenceOfImages(image_folder, saveTo_folder, NameOfStack, ref_annotation_stack_cells,section):
    global videoSpeed
    global mouseX, mouseY
    global pauseFlag
    global rearFlag
            ...
            for stack in Stacks:
                ...
                if (stack.startswith(stackNameStartsWith) and os.path.isdir(os.path.join(path2Case, section, stack))):
                    ...
                    IS_VALID_STACK = ReadSequenceOfImages(os.path.join(path2Case, section, stack), saveTo_folder,
                                                          NameOfStack,
                                                          ref_annotation_stack_cells,section)
