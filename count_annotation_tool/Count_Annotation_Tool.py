...
def key(event):
    ...
    if event.keysym == "Up":
        ...
        canvas.create_line(x - 5, y + 5, x + 5, y - 5, width=3, fill="#000000")
        canvas.create_line(x - 5, y - 5, x + 5, y + 5, width=3, fill="#000000")
        canvas.update()
...
def ReadSequenceOfImages(image_folder, NameOfStack):
    global videoSpeed
    global mouseX,mouseY
    global pauseFlag
    global rearFlag
    ...
    try:
        if MODE == 'new':
            ...
            refImg = cv2.imread(os.path.join(REF_IMG_DIR, NameOfStack + '_pred.png'))
        else:
            print('Invalid mode. Please select "new" for new annotation and "update" to update already existing '
              'annotation based on prediction from pre-trained model.')
            sys.exit()
